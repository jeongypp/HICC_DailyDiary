...
public class DiaryDetailResponse {
    ...
    private List<String> domainNames; //[cite: 7]

    // 점수 배열(가중치 반영X): 오각형 레이더 차트 가독성 위함
    @Schema(description = "점수 배열", example = "[8, -5, 0, 7, 9]")
    private List<Integer> domainScores; //[cite: 7]

    @Schema(description = "추가 메모", example = "오늘은 비가 와서 기분이 처진다.")
    private String memo; //[cite: 7]

    @Schema(description = "AI 피드백 내용", example = "학업에 집중하지 못했지만, 운동으로 활력을 찾은 하루군요!")
